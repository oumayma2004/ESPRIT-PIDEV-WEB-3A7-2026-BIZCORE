...
class AdminCoachController extends AbstractController
{
    ...
    {
        // Handle AJAX JSON submission from modal
        if ($request->isXmlHttpRequest() && $request->isMethod('POST')) {
            ...
            $coach->setNom($data['nom'] ?? $coach->getNom());
            $coach->setPrenom($data['prenom'] ?? $coach->getPrenom());
            $coach->setBiographie($data['biographie'] ?? $coach->getBiographie());
            $coach->setExperienceAnnees(isset($data['experienceAnnees']) ? (int) $data['experienceAnnees'] : $coach->getExperienceAnnees());
            $coach->setTarifHoraire(isset($data['tarifHoraire']) ? $data['tarifHoraire'] : $coach->getTarifHoraire());
            $coach->setDisponibilite($data['disponibilite'] ?? $coach->getDisponibilite());
            $coach->setNumTel($data['numTel'] ?? $coach->getNumTel());

            // Check duplicate (excluding self)
            $existing = $repo->findByFullName($coach->getNom(), $coach->getPrenom(), $coach->getId());
            if ($existing) {
                return $this->json(['success' => false, 'message' => 'Un autre coach avec ce nom et prénom existe déjà.']);
            }
    ...
    // MODAL DATA — return coach fields as JSON for the edit modal
    #[Route('/{id}/modal-data', name: 'modal_data', methods: ['GET'], requirements: ['id' => '\d+'])]
    public function modalData(Coach $coach, EntityManagerInterface $em): JsonResponse
    {
        $domaines = $em->getConnection()->fetchAllAssociative('SELECT nom_domaine FROM domaine_coaching ORDER BY nom_domaine');
        $domaineList = array_map(fn($r) => $r['nom_domaine'], $domaines);
        ...
        return $this->json([
            ...
            'numTel'          => $coach->getNumTel(),
            'domaineChoices'  => $domaineList,
        ]);
    }
    ...
    public function delete(Request $request, Coach $coach, EntityManagerInterface $em): Response
    {
        if ($this->isCsrfTokenValid('delete'.$coach->getId(), $request->getPayload()->getString('_token'))) {
            ...
            // Delete child rows from tables that reference coach
            $conn->executeStatement('DELETE FROM reservation WHERE id_coach = ?', [$coachId]);
            $conn->executeStatement('DELETE FROM rating WHERE id_coach = ?', [$coachId]);
            $conn->executeStatement('DELETE FROM disponibilite WHERE id_coach = ?', [$coachId]);
