...
class ApiController extends AbstractController
{
    ...
    public function translateBatch(Request $request): JsonResponse
    {
        ...
        // Join texts using a delimiter that is usually kept intact by translations
        $delimiter = ' ||| ';
        ...
        try {
            ...
            // Split back taking into account possible whitespace around the delimiter
            ...
            foreach ($texts as $i => $original) {
                if (isset($translatedTexts[$i]) && trim($original) !== '') {
                    ...
                    $result[] = $original; // Fallback to original
                }
            }
    ...
    {
        try {
            // Fetch rates from base EUR using free API
